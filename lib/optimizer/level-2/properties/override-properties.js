...
function turnIntoMultiplex(property, size) {
  ...
  for (var i = 0, l = property.components.length; i < l; i++) {
    ...
    var defaultValue = compactable[component.name].defaultValue;

    for (var j = 1; j < size; j++) {
      component.value.push([Token.PROPERTY_VALUE, Marker.COMMA]);

      if (compactable[component.name].multiplexLastOnly) {
        Array.prototype.push.apply(component.value, value);
      } else {
        if (Array.isArray(defaultValue)) {
          for (var k = 0; k < defaultValue.length; k++) {
            component.value.push([Token.PROPERTY_VALUE, defaultValue[k]]);
          }
        } else {
          component.value.push([Token.PROPERTY_VALUE, defaultValue]);
        }
      }
    }
  }
